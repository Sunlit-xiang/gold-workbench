...
> **Intraday/premarket routing:** For 盘前分析、今日市场背景、US equities/Gold/Forex/Crypto session preparation, use [skills/intraday-market-context/SKILL.md](skills/intraday-market-context/SKILL.md). That workflow keeps Codex-led signal routing but removes probability forecasts and all trade-entry analysis.
...
- Eastmoney throttles per source IP; quotes / fund flow / sectors fall back to its delay host automatically. `get_history` sources from Tencent first and only falls back to Eastmoney, since Eastmoney's history host is the one it throttles hardest — that means `turnover_cny` is `None` on bars that came from Tencent, but the OHLCV is complete
