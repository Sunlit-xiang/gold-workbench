...
## Premarket Market Core（盘前全球核心）

```python
from digital_oracle import MarketSnapshotQuery, YahooMarketCoreProvider

snapshot = YahooMarketCoreProvider().get_snapshot(
    MarketSnapshotQuery("dxy", "美元指数 DXY", "DX-Y.NYB")
)

# snapshot.current / previous_close / session_open
# snapshot.since_previous_close_pct / today_change_pct
# snapshot.market_time / fetched_at / freshness
```

该 Provider 只构造带时间语义的盘前快照，不做分钟K线技术分析。

## Economic Calendar（结构化经济日历）

```python
from digital_oracle import EconomicCalendarQuery, TradingEconomicsCalendarProvider

calendar = TradingEconomicsCalendarProvider()  # TRADING_ECONOMICS_API_KEY
...
事件字段包括 UTC 时间、时间精度（Trading Economics `DateSpan` 映射为 `exact / estimated / unknown`）、货币、重要度、Actual、Forecast、Previous、来源与最后更新时间。API key 未设置时抛出明确的 `ProviderError`，调用方应显示降级状态。

## Unified Premarket Bundle（统一盘前数据包）

```python
from digital_oracle import IntradayContextQuery, IntradayMarketContextService

bundle = IntradayMarketContextService().build(
    IntradayContextQuery(target="gold", session="london")
)
payload = bundle.to_dict()

# core / target_specific / events / derivatives
# data_quality / errors
```
